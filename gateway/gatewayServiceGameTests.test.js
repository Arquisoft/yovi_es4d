...
describe('POST /api/game/start', () => {
    test('inicia una partida y devuelve gameId con token válido', async () => {
        axios.post.mockResolvedValueOnce(gameStartOk);

        const res = await request(server)
            .post('/api/game/start')
            .set('Cookie', createCookie('j1'))
    ...
    test('devuelve 401 si falta el token', async () => {
        const res = await request(server)
            .post('/api/game/start')
            .send({ gameMode: 'vsBot', botMode: 'random_bot' });
...
describe('GET /api/game/:gameId', () => {
    test('devuelve el estado de la partida con token válido', async () => {
    ...
    test('devuelve 404 si el game-service falla', async () => {
